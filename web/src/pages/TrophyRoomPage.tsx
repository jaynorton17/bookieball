...
function bestStreak(cards: TrophyCard[]): { teamName: string; competition: string; streak: number } | null {
  ...
  cards.forEach((card) => {
    ...
    let previousTeam = ''; let previousSeason = -99; let streak = 0;
    rows.forEach((row) => {
      const season = seasonNumber(row.season);
      if (row.teamName === previousTeam && season === previousSeason + 1) streak += 1; else streak = 1;
      if (!best || streak > best.streak) best = { teamName: row.teamName, competition: card.title, streak };
      previousTeam = row.teamName; previousSeason = season;
    });
  });
  return best;
}
function longestWait(rows: TrophyWinner[]): { teamName: string; seasons: number } | null {
  ...
  rows.forEach((row) => { const seasons = byTeam.get(row.teamName) ?? []; seasons.push(seasonNumber(row.season)); byTeam.set(row.teamName, seasons); });
...
function HonourHero({ label, record, team, trophy }: { label: string; record: [string, number] | null; team?: TeamMeta; trophy: 'cup' | 'super' | 'master' }) {
  ...
  return (
    <section className="page page-dashboard competition-page competition-page-trophy">
      <div className="competition-page-shell">
        ...
        <div className="trophy-honour-heroes">
          <HonourHero label="MOST DECORATED" record={mostDecorated} team={mostDecorated ? teamByName.get(mostDecorated[0]) : undefined} trophy="super" />
          <HonourHero label="CUP KING" record={cupKing} team={cupKing ? teamByName.get(cupKing[0]) : undefined} trophy="cup" />
          <HonourHero label="LEAGUE KING" record={leagueKing} team={leagueKing ? teamByName.get(leagueKing[0]) : undefined} trophy="master" />
        </div>

        <div className="trophy-record-strip">
          <article><span>Longest title streak</span><strong>{streak ? `${streak.teamName} · ${streak.streak} straight` : '—'}</strong><small>{streak?.competition ?? ''}</small></article>
          <article><span>Longest wait between honours</span><strong>{wait ? `${wait.teamName} · ${wait.seasons} seasons` : '—'}</strong></article>
          <article><span>Latest honour</span><strong>{mostRecent ? `${mostRecent.season} · ${mostRecent.teamName}` : '—'}</strong><small>{mostRecent?.competition ?? ''}</small></article>
        </div>

        <div className="trophy-cabinet">{cards.map((card) => <details key={card.key} className={`trophy-shelf tone-${card.tone}`}><summary className="trophy-shelf-head"><CompetitionTrophyMark variant={card.trophy} className="trophy-shelf-mark" /><div><h3>{card.title}</h3><p>{card.winners.length ? `${card.winners.length} honours · click for archive` : 'No winner yet'}</p></div></summary><div className="trophy-recent-winners">{card.winners.length === 0 ? <span className="muted">No winners yet.</span> : card.winners.map((winner, index) => { const team = teamByName.get(winner.teamName); return <div key={`${card.key}-${winner.season}-${winner.teamName}-${index}`} className="trophy-recent-row"><span>{winner.season}</span><TeamBadge name={winner.teamName} ballColor={team?.ballColor ?? null} ringColor={team?.ringColor ?? null} textColor={team?.textColor ?? null} size={18} /><strong>{winner.teamName}</strong></div>; })}</div></details>)}</div>
