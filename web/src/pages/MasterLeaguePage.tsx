...
export function MasterLeaguePage() {
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? `Master League API unavailable: ${error.message}`
          : 'Master League API unavailable. Restart the backend and try again.',
      );
    } finally {
      setLoading(false);
    }
  ...
  const fixturesByGw = useMemo(() => {
    ...
    fixtures
      .slice()
      .sort((a, b) => gwNumber(a.gw) - gwNumber(b.gw) || a.id - b.id)
      .forEach((fixture) => {
  ...
  const nextGw = currentGwIndex >= 0 && currentGwIndex < GAMEWEEKS.length - 1
    ? GAMEWEEKS[currentGwIndex + 1]
    : null;
  ...
  const runGenerate = async (fromGw: string, toGw: string) => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'Unable to generate Master League fixtures.');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const generateUpcoming = async () => {
    if (!nextGw) {
      setMessage('No upcoming gameweeks left to generate.');
      return;
    }
  ...
  const generateSelectedRange = async () => {
    if (!rangeValid) {
      setMessage('Choose a valid gameweek range (From must be before or equal to To).');
      return;
    }
